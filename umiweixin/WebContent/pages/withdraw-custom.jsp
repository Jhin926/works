...
<div class="trade-class">
  <img src="<%=basePath %>images/logo-custom.png" width="40" alt=""/>
  悠米定制
</div>
<div class="trade-method">请选择转出至:</div>
<c:if test="${empty bankInfo }">
  <a href="<%=basePath%>bankCardAction!loadUserBankInfo.action" class="pay-card">您还没有设置默认银行卡,请点击这里设置</a>
</c:if>
<input id="finaAcc" type="hidden" value="${finaAcc.id}"/>
<c:if test="${!empty bankInfo }">
  <ul class="bank-info-container">
    <li class="bank-info">
      <p class="left">
    		<span class="radio checked">
    			<input type="radio" name="aim"/>
    		</span>
      </p>

      <p class="bank-logo2 left"><img src="<%=basePath %>images/${bankInfo.bankPic }" width="40" alt=""/></p>
    ...
    <li class="bank-info">
      ...
      <p class="bank-logo2 left"><img src="<%=basePath %>images/index_09.png" width="36" alt=""/></p>

      <div class="left">
        <p class="account-name">悠米活期</p>
      </div>
      <div class="clear"></div>
    </li>
    <li class="bank-info">
  ...
  <form id="search" name="search" method="post" action="<%=basePath%>withdrawAction!asseAmount.action">
    <div class="common-input">
      ...
      <input id="withdrawAmount" style="width: 60%;font-size: 2.2rem;" class="withdraw-input" type="number" name="amount" placeholder="您最多可转出${withdrawalAmount }元"/>
    ...
    <div class="sub sub-recharge" style="display: none;">
      ...
      我已阅读并同意<a id="agreement" data-username="${user.userName}" data-href="terms_dz.html">《悠米定制用户投资协议》</a>
    </div>
...
<script>
  ...
  $(function () {
    ...
    var hasLimit = false//是否已经选择期限
      ,isValue= false//是否是合法值
      ,aim = 0//转出的目标
      ,canAjax = false;

    var getSearch = location.search.substring(1);
    if (getSearch.indexOf("timeLimit") >= 0) {//如果是选择期限之后来的
      hasLimit = true;
      aim = 2;
    ...
    $("#withdrawAmount").keyup(function (){//判断输入的是否是合法值
      var getVal = $(this).val();
        ...
        if (aim == 0) {
          ...
          isLegal(getVal,aim);
          if ($(".checkbox-recharge").hasClass("checked-recharge") && isValue) {
            canClick(true);
          }else{
            canClick();
          }
    ...
    $(".btn-all").on("touchend", function () {//全部转出
      var getMaxAmount = Number($("#maxAmount").text());
      $(".withdraw-input").val(getMaxAmount);
      isValue = true;
    ...
    $(".bank-info").on("touchend", function () {//选择转出类型
      ...
      if (aim == 0) {//转出到银行
        ...
        $("#agreement").text("《悠米活期用户投资协议》").attr("data-href", "terms_hq.html");
      } else if (aim == 2) {
        ...
        $("#agreement").text("《悠米定制用户投资协议》").attr("data-href", "terms_dz.html");
      }
      ;
    });
    ...
    function withDraw() {
      ...
      if (aim == 0){//转出到银行卡
        ...
        var limit = $(".account-name").eq(1).text().substring(5);
        var data = {"srcID": 888, "dstID": 400, "finaAccId": $("#finaAcc").val(), "amount": $("#withdrawAmount").val(), "timeLimit": '', "accountID": "888"};
        if (aim == 2) {
          data = {"srcID": 888, "dstID": 888, "finaAccId": $("#finaAcc").val(), "amount": $("#withdrawAmount").val(), "timeLimit": limit.substring(0, limit.length - 1), "accountID": "888"};
        }
