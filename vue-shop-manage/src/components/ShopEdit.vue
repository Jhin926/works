    ...
    <ul class="pro-input">
      ...
      <li>
        ...
        <div>
          ...
          <input class="middle" type="text" id="pro-sort" v-model="teaObj.sortNo" />
        </div>
      </li>
      <li>
        <label><span>*</span>首图</label>
        <ul class="btn-upload">
          <!--<li class="left"><img :src="teaObj.indexPicture" width="100" height="100" /></li>-->
          <li class="left" v-if="indList.length>0"><img :src="indList[0]" width="100" height="100" /></li>
          <div class="clear"></div>
      ...
      <li>
        ...
        <draggable class="btn-upload des-upload" :list="imgList">
          <li v-for="(img, index) in imgList" class="left del-cont">
            <span class="img-del" v-on:click="removeImg(index)"><img src="../assets/icon-del2.png"></span>
            <img :src="img" width="100" height="100" />
          </li>
          <div class="clear"></div>
          <li><label for="upload"><img src="../assets/img-upload.png" width="100" height="100" alt="" /></label></li>
        </draggable>
        <p class="upload-prompt clear">支持jpg、jpeg、png图片格式，图片大小不能超过3MB，宽度不超过640px</p>
      ...
      <li>
        ...
        <draggable class="btn-upload des-upload" :list="depictList">
          <li v-for="(img, index) in depictList" class="left del-cont">
            <span class="img-del" v-on:click="removeDes(index)"><img src="../assets/icon-del2.png"></span>
            <img :src="img" width="100" height="100" />
          </li>
          <div class="clear"></div>
          <li>
...
export default {
  ...
  beforeCreate () {
    ...
    this.$http.get('api/tea/info?id=' + id).then(function (res) {
      if (res.body.code === 0) {
        this.$data.teaObj = res.body.data
        delete this.$data.teaObj.weightText
        this.$data.indList.unshift(this.$data.teaObj.indexPicture)
        for (var i = 0; i < this.$data.teaObj.teaPictures.length; i++) {
          this.$data.imgList.push(this.$data.teaObj.teaPictures[i].pictureUrl)
        }
        for (var j = 0; j < this.$data.teaObj.teaPicturesShowResults.length; j++) {
          this.$data.depictList.push(this.$data.teaObj.teaPicturesShowResults[j].pictureUrl)
        }
      } else {
        window.alert(res.body.message)
      }
  ...
  methods: {
    ...
    removeImg: function (_img) {
      this.$data.imgList.splice(_img, 1)
      return this.$data.imgList
    },
    removeDes: function (_img) {
      this.$data.depictList.splice(_img, 1)
      return this.$data.depictList
    },
    publish: function () {
      if (this.$data.imgList.length > 0 && this.$data.imgList.length > 0 && this.$data.imgList.length > 0) {
        this.$data.teaObj.indexPicture = this.$data.indList[0]
