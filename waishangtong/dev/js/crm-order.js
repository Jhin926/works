require([ 'common' ], function () {
    require(['maintab', 'blockUI', 'datetimepickerCN'], function (maintab) {
        ...
        //  打开删除面板
        $content.on('click', '#btn-dels', function (e) {
            $.EventFn(e);
            $.modalsFn($('#delsModal'));
        });
        //  关闭面板
        ...
        $content.on('click', '#btn-del-ok', function () {
            var ids = orderObj.getAllChecked().ids;
            orderObj.changeQuoStatus(ids, 0);
        });
        //  查看报价详情
        ...
        $content.on('click', '.page_info thead>tr>th', function (e) {
            ...
            if (_index == 6) {
                ...
                if ($i.hasClass('s-orderList')) {
                    ...
                    orderObj.filterShow(list, $.filterObj.total);
                } else {
                    $.filterObj.type = 0;
        ...
        var orderObj = {
            filter: function (obj) {
                ...
                $.ajax({
                    url: Base.sitUrl + '/api/order/v1/order/list',
                    data: data,
                    type: 'POST',
                    dataType: 'json',
                ...
                if (list.length > 0) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        var name = list[i].name,
                            customerName = list[i].customerName || '匿名',
                            createUserName = list[i].createUserName || '匿名';
                        if (name.length >= 25) {
                            name = name.substr(0, 25) + '...';
                        }
                        html += '<tr>\
                                    <td><div class="checker"><span><input type="checkbox" name="batch" data-id="' + list[i].id + '"></span></div></td>\
                                    <td><span>' + list[i].orderNo + '</span></td>\
                                    <td><span>' + name + '</span></td>\
                                    <td class="customer-card" style="position:relative"><span class="s-special customer-card-name" data-id="' + list[i].customerId + '">' + customerName + '</span><div class="sender-condition-style"></div></td>\
                                    <td><span>' + list[i].orderAmount + ' USD</span></td>\
                                    \<td><span>' + createUserName + '</span></td>\
                    ...
                    var none = '<div class="trNone">' +
                        ...
                        '<p class="trnone-text">暂无订单</p>'+
                        '</div>' +
                        '<a href="pop-order-add.html" class="trnone-btn btn btn-primary" data-code="order_add" data-maintab></i><span>创建订单</span></a>'+
                        '</div>';
                    $('.trNone').remove();
