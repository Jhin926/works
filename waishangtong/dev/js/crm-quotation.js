    ...
    require(['maintab', 'blockUI', 'datetimepickerCN'], function (maintab) {
        ...
        $content.on('click', '.m_batch>button', function (e) {
            ...
            if (type == 'dels') {
                ...
                var ids = quoObj.getAllChecked().ids;
                var Ids = '';
                if (ids.length >= 2) {
                    $.Alert('只能单个文件发送')
                    return
        ...
        $content.on('click', '.page_info tbody>tr', function (e) {
            ...
            if (!isNaN(id)) {
                maintab.showTab(Base.url + '/html/pop-quotation-detail.html?id=' + id + "&v=" + window.ver, '报价详情');
            }
        });
        //  通过报价单名快速搜索
        ...
        $content.on('click', '.page_info thead>tr>th', function (e) {
            ...
            if (_index == 6) {
                ...
                if ($i.hasClass('s-orderList')) {
                    ...
                    quoObj.filterShow(list, $.filterObj.total);
                } else {
                    $.filterObj.type = 0;
        ...
        var quoObj = {
            ...
            filter: function (obj) {
                ...
                $.ajax({
                    url: Base.sitUrl + '/api/quotation/v1/quotation/list',
                    data: data,
                    type: 'POST',
                    dataType: 'json',
            ...
            /*
             * @changeQuoStatus 改变报价单状态
             * @ids 订单IDS
             * @type  DEL(0, "删除")
             */
            changeQuoStatus: function (ids, type) {
                $.ajax({
                    url: Base.sitUrl + '/api/quotation/v1/quotation/batch/delete',
                    data: {
                        ids: ids.join(';')
                    },
                    type: 'POST',
                ...
                if (list.length > 0) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        html += '<tr>\
                                    ...
                                    <td><span>' + list[i].quotationNo + '</span></td>\
                                    <td><span>' + name + '</span></td>\
                                    <td class="customer-card" style="position:relative"><span class="s-special user-name customer-card-name" data-id="' + list[i].customerId + '">' + (list[i].customerName || '匿名') + '</span><div class="sender-condition-style"></div></td>\
                                    <td><span>' + list[i].totalPrice + ' USD</span></td>\
                    ...
                    var none = '<div class="trNone">' +
                        ...
                        '<img src="../images/empty-quo.png" alt="" />' +
                        '<p class="trnone-text">暂无报价单</p>'+
                        '</div>' +
                        '<a href="pop-quotation-add.html" class="trnone-btn btn btn-primary" data-code="quotation_add" data-maintab></i><span>创建报价单</span></a>'+
                        '</div>';
                    $('.trNone').remove();
        ...
        $filterModal.on('click', '#screen', function (e) {
            ...
            screenOpt = inputOpt;
            //if(!($name.val() =='' && $custInfo.val() =='' && $dateBeg.val() =='' && $dateEnd.val() =='')){//非所有为空的情况
                quoObj.filter(screenOpt);//如果没有输入任何值，那就过滤全部
            //}
