...
require([ 'common' ], function () {
    require(['maintab', 'blockUI', 'datetimepickerCN', 'jqprint', 'ztree'], function (maintab) {
        ...
        $('.email-detail-main-content').attr('src', Base.sitUrl + '/html/read.html?type=2&id=' + emailId + "&v=" + window.ver)
        ...
        $('#reEmail').on('click', function (e) {
            $.EventFn(e);
            var id = emailId;
            maintab.showTab(Base.url + '/html/pop-email-newOut.html?type=1&showType=right&id=' + id + "&v=" + window.ver, '邮件回复');
        });
        ...
        $('#allReEmail').on('click', function (e) {
            $.EventFn(e);
            var id = emailId;
            maintab.showTab(Base.url + '/html/pop-email-newOut.html?type=1&showType=right&id=' + id + "&v=" + window.ver, '全部回复');
        });
        ...
        $('#zfEmail').on('click', function (e) {
            $.EventFn(e);
            var id = emailId;
            maintab.showTab(Base.url + '/html/pop-email-newOut.html?type=2&showType=right&id=' + id + "&v=" + window.ver, '邮件转发');
        });
        ...
        $('.sender-info').on('click', '.sender-condition>.condition-cz-btn>.fayoujian', function (e) {
            $.EventFn(e);
            var id = emailId;
            maintab.showTab(Base.url + '/html/pop-email-newOut.html?type=0&showType=right&id=' + id + "&v=" + window.ver, '发邮件');
        });
        ...
        //点击加好，添加标签
        $('.email-detail').on('click', '.add-mark-icon', function (e) {
            $.EventFn(e);
            if ($('.caozuo-down-show').css("display", "none")) {
                $('.caozuo-down-show').show();
                $('.caozuo-down-show>li').eq(5).children('.modals').addClass('active');
                detailObj.inboxMenu();
            } else {
                $('.caozuo-down-show').hide();
                $('.caozuo-down-show>li').eq(5).children('.modals').removeClass('active');
            }
        });
        ...
        $('.email-detail').on('click', '.caozuo-down-show>li', function (e) {
            ...
            if (inn == 0) {
                var id = emailId;
                maintab.showTab(Base.url + '/html/pop-email-newOut.html?type=3&showType=right&id=' + id + "&v=" + window.ver, '编辑邮件');
            } else if (inn == 1) {
                var id = emailId;
                var data = {
                    id: emailId,
                ...
                data = JSON.stringify(data)
                maintab.showTab(Base.url + '/html/pop-email-newOut.html?type=10&showType=right&id=' + id + '&data=' + data + "&v=" + window.ver, '作为附件转发');
            } else if (inn == 2) {
                var data = {
                    id: emailId,
                    folder: $('.subject').attr('data-folder'),
                ...
                var emailName = $('.detailSender').val();
                ...
                detailObj.deleteEmail(emailId, $oThis);
                $('.caozuo-down-show').hide();
            } else if (inn == 7) {
                detailObj.rubishEmail(emailId, 1);
                $('.caozuo-down-show').hide();
            } else if (inn == 8) {
                $(".caozuo-down-show").hide();
                window.print();
        ...
        //  新建分组
        $('.caozuo-down-show').on('click', '#btn-addTabs', function (e) {
            $.EventFn(e);
            var name = $('#addTabName').val();
            if (name != null && name != '') {
                detailObj.addGroups(name);
                $('#addTabName').val('');
            } else {
                confirm("请输入分组名！");
            }
        ...
        //显示新建标签
        $('.emailcreattag-t').on('click', function (e) {
            $.EventFn(e);

            var stabs = $(this).find('.s-updownBg');
            if (stabs.hasClass('s-add')) {
                $(this).find('.s-add').removeClass('s-add').addClass('s-close');
                $(this).parent('.emailcreattag').find('.emailcreattag-b').show();
            } else {
                $(this).find('.s-close').removeClass('s-close').addClass('s-add');
                $(this).parent('.emailcreattag').find('.emailcreattag-b').hide();
            }
        });
        //新建标签
        $('.caozuo-down-show').on('click', '#btn-addTags', function (e) {
            $.EventFn(e);
            var name = $('#addTagName').val();
            if (name != null && name != '') {
                detailObj.addEmailLabel(name);
        ...
        var detailObj = {
            emailDetail: function (obj) {
                ...
                if (tags == null || tags == '') {
                    ...
                    for (var i = 0; i < tags.length; i++) {
                        html3 += '<li class="mark-p" data-id="' + tags[i].id + '" style="border:1px solid #ff5720;border-radius:8px;">\
            ...
            rubishEmail: function (emailId, value) {
                ...
                var postData = 'data=' + JSON.stringify(data);
                $.ajax({
                    url: Base.sitUrl + '/api/email/outbox/v1/spam',
                    type: 'POST',
                    data: postData,
                    success: function (res) {
                        if (!res.success) {
            ...
            addGroups: function (name) {
                ...
                var dataJson = '{"valueEnters":[{"value": "' + name + '"}]}';
                var postData = 'data=' + JSON.stringify(data);
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        $('#addTabName').val('');
                        detailObj.inboxMenu(data.emailGroup);
                    }
                });
            },
            //分配分组
            ...
            distributionTag: function (emailId, tagId, tagName) {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        var html = '<li class="mark-p" data-id="' + tagId + '" style="border:1px solid #ff5720;border-radius:8px;">' +
            ...
            //删除邮件
            deleteEmail: function (ids, oThis) {
                var data = {
                    'ids': [ids]
                };
                var postData = 'data=' + JSON.stringify(data);
                $.ajax({
                    url: Base.sitUrl + '/api/email/outbox/v1/delete',
                    type: 'POST',
                    data: postData,
                    success: function (res) {
                        if (!res.success) {
